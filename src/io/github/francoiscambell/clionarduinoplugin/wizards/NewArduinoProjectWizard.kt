...
class NewArduinoProjectWizard : CMakeProjectWizard("New Arduino Sketch Project", "NewArduinoSketchProject") {
    ...
    override fun tryFinish(): Boolean {
        ...
        if (projectRootDir.exists()) {
            val fileList = projectRootDir.list { dir, name ->
                !".DS_Store".equals(name, ignoreCase = true) && !"Thumbs.db".equals(name, ignoreCase = true)
            }
            if (fileList != null && fileList.size > 0) {
                val dialogAnswer = Messages.showYesNoDialog("Directory $projectRootPath already exists and not empty.\nWould you like to continue?",
    ...
    private fun createProject(projectName: String, projectRootPath: String): String {
